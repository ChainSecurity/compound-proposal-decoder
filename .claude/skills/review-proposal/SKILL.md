...
**You are allowed and encouraged to modify the proposal decoder** in this repository (`src/`) during the review if needed to:
...
1. Clone the proposal-simulator repository
2. Create a Tenderly virtual testnet (fork of mainnet)
3. Configure `proposal.config.json` with your Tenderly RPC URL:
...
# Simulate an existing proposal
npx tsx proposal.ts $ARGUMENTS

# Simulate with persistence (creates snapshot)
npx tsx proposal.ts $ARGUMENTS --persist

# Simulate each action separately (useful for debugging)
npx tsx proposal.ts $ARGUMENTS --separately
```
...
| Check | Result | Source |
|-------|--------|--------|
| Simulation Status | Success | `npx tsx proposal.ts $ARGUMENTS` |

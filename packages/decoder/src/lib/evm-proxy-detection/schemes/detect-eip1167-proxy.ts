...
const parse1167Bytecode = (bytecode: unknown): `0x${string}` => {
...
async function detect(
  ...
  try {
    const bytecode = await jsonRpcRequest({
      method: "eth_getCode",
      params: [proxyAddress, blockTag],
    });
    const target = readAddress(parse1167Bytecode(bytecode));
    return {
      target,
      type: ProxyType.Eip1167 as const,
      immutable: true,
    };

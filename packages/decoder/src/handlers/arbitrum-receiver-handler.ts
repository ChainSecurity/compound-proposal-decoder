...
/**
 * ArbitrumBridgeReceiver (Arbitrum) — receiving side
 *
...
const KNOWN_RECEIVERS: Record<string, { label: string }> = {
  [checksum("0x42480C37B249e33aABaf4c22B20235656bd38068")]: {
    label: "ArbitrumBridgeReceiver (Arbitrum)",
  },
};
...
export const arbitrumReceiverHandler: Handler = {
  name: "ArbitrumBridgeReceiver (queued actions)",
  match: (ctx: RegistryCtx) => {
    if (ctx.chainId !== 42161) return false;
  ...
  expand: (ctx: RegistryCtx) => {
    ...
    if (!decoded) {
      logger.warn({ data }, "Failed to decode ArbitrumBridgeReceiver payload");
      return [];
    }

...
import type { AddressMetadataMap, CallInsight, CallNode, DecodedProposal } from "@/types";
...
function renderAddressValue(value: unknown, metadata?: AddressMetadataMap): string {
  ...
  try {
    ...
    push(info.contractName ?? undefined);
    push(info.etherscanLabel ?? undefined);
    push(info.tokenSymbol ?? undefined);
    push(info.ensName ?? undefined);
    ...
    if (info.implementation) {
      ...
      pushImpl(info.implementation.contractName ?? undefined);
      pushImpl(info.implementation.etherscanLabel ?? undefined);
      pushImpl(info.implementation.tokenSymbol ?? undefined);
      if (info.implementation.labels && info.implementation.labels.length) {
        pushImpl(info.implementation.labels[0]);
      }
      pushImpl(info.implementation.ensName ?? undefined);

      rendered += ` ${chalk.gray(`→ ${info.implementation.address}`)}`;
      if (implParts.length) {
...
function edgeLabel(edge: {
  ...
  if (edge.type === "bridge") {
    return `Bridge → chain ${edge.chainId ?? "?"}${edge.label ? ` (${edge.label})` : ""}`;
  }
  if (edge.type === "multicall") {
    const idx = typeof edge.meta?.index === "number" ? ` #${edge.meta.index}` : "";
    return `Multicall${idx}`;
  }
  if (edge.type === "delegatecall") return "Delegatecall";
  if (edge.type === "staticcall") return "Staticcall";
  return edge.label || edge.type || "child";
}
...
function renderInsight(insight: CallInsight, width: number): string {
  ...
  insight.entries.forEach(({ label, value }) => {
    ...
    const line = `${header} ${chalk.white(value)}`;
    if (stringWidth(line) <= width - 4) {
      lines.push(line);
    } else {
      lines.push(header);
      lines.push(
        ...wrapLines([chalk.white(value)], width - 4).map((l) => `${" ".repeat(2)}${l}`)
      );
    }
  });
...
function renderNodeBody(node: CallNode, contentWidth: number): string {
  ...
  lines.push(
    ...wrapLines([`${chalk.cyan("Value:")}    ${node.valueWei.toString()} wei`], contentWidth)
  );
  lines.push(
    ...wrapLines([`${chalk.cyan("Selector:")} ${selectorOf(node.rawCalldata)}`], contentWidth)
  );
  lines.push(...formatCalldata(node.rawCalldata, contentWidth));
  lines.push(""); // spacer
  ...
  if (node.decoded) {
    ...
    lines.push(
      ...wrapLines(
        [`${chalk.cyan("Function:")} ${node.decoded.name}(${argSignature})`],
        contentWidth
      )
    );
